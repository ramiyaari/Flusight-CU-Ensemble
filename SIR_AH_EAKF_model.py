...
# SIR model equations
def sir_ah_step(S, I, LowR0, HighR0, gamma, N, AH):
    
    a = -180
    b = np.log(HighR0-LowR0)
    beta = (np.exp(a*AH+b) + LowR0)*gamma
...
# Main SIR-AH-EAKF model
def sir_ah_eakf(num_ensembles, N, S_min, S_max, I_min, I_max, 
             LowR0_min, LowR0_max, HighR0_min, HighR0_max, 
             gamma_min, gamma_max, AH, 
    ...
    Parameters:
        ...
        LowR0_min, LowR0_max: float
            Minimum and maximum values for initializing LowR0.
        HighR0_min, HighR0_max: float
            Minimum and maximum values for initializing LowR0.
        gamma_min, gamma_max: float
            Minimum and maximum values for initializing gamma.
        AH:
            Absolute humidity values used to modulate R0.
    ...
    LowR0_ensemble = np.random.uniform(LowR0_min, LowR0_max, num_ensembles)
    HighR0_ensemble = np.random.uniform(HighR0_min, HighR0_max, num_ensembles)
    gamma_ensemble = np.random.uniform(gamma_min, gamma_max, num_ensembles)

    min_values = [0, 0, LowR0_min, HighR0_min, gamma_min]
    max_values = [N, S_max, LowR0_max, HighR0_max, gamma_max]
    ...
    for t in range(obs_len):
        ...
        for d in range(7):
            ...
            S, I, new_infected = sir_ah_step(S, I, LowR0_ensemble, HighR0_ensemble, 
                                            gamma_ensemble, N, AH[t*7+d])
        ...
        # EAKF update
        state_ensembles = np.array([S, I, LowR0_ensemble, HighR0_ensemble, gamma_ensemble])
        ...
        S, I, LowR0_ensemble, HighR0_ensemble, gamma_ensemble = updated_ensembles
...
def generate_sir_ah_eakf_pred(df, start_date, ref_date, weeks_to_predict, 
                                locations, AH, quantiles, num_samples, 
                                dat_changerate_ref, basedir, model_desc='SIR-EAKF'):
    
    N = 1e5 
    S_min, S_max = 0.2 * N, 0.8 * N #0.2 * N, 0.6 * N #
    # I_min, I_max = 1, 200
    LowR0_min, LowR0_max = 0.8, 1.3
    HighR0_min, HighR0_max = 1.3, 4
    ...
    for loc_abbr in locations_abbr:
        ...
        # Extract AH for location and rearrange to start from starting day of year
        AH_loc = AH[loc_abbr]
        AH_loc = np.concatenate((AH_loc[(start_doy-1):], AH_loc[:(start_doy-1)]))
        ...
        obs_I_ens = sir_ah_eakf(num_samples, N, S_min, S_max, I_min, I_max, 
                                LowR0_min, LowR0_max, 
                                HighR0_min, HighR0_max,
                                gamma_min, gamma_max, 
                                AH_loc, weeks_to_predict, obs_data, obs_var, 
